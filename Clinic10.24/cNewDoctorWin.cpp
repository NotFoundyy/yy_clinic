...
int NewDoctorWin_init(){
	...
	getTime(53,4);
	winRun(&newDoctorWin);
	if(newDoctorWin.flag == 7){
		DOC newDoc = {0};
		strcpy(newDoc.docID,newDoctorWin.arr[9].context);
		strcpy(newDoc.docName,newDoctorWin.arr[10].context);
		strcpy(newDoc.docHospital,newDoctorWin.arr[11].context);
		strcpy(newDoc.docClass,newDoctorWin.arr[12].context);
		strcpy(newDoc.docRole,newDoctorWin.arr[13].context);
		strcpy(newDoc.docAsk,newDoctorWin.arr[14].context);
		doc[docNum++] = newDoc;
		return 11;
	}
	return 0;
