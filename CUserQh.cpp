...
CUserQh::CUserQh(int winX,int winY,int winWidth,int winHeight):CWindow(winX,winY,winWidth,winHeight)
{
	...
	this->shurulab=new CLabel(30,10,0,0,"输入就诊日期",LABEL);
	...
	this->yybhlab=new CLabel(25,14,0,0,"预约编号",LABEL);
	this->yysjlab=new CLabel(38,14,0,0,"预约时间",LABEL);
	this->yyjzsjlab=new CLabel(48,14,0,0,"预约就诊时间",LABEL);
	this->yyyylab=new CLabel(63,14,0,0,"预约医院",LABEL);
	this->yykslab=new CLabel(76,14,0,0,"预约科室",LABEL);
	this->yyyslab=new CLabel(89,14,0,0,"预约医生",LABEL);
	this->jzztlab=new CLabel(102,14,0,0,"就诊状态",LABEL);
	this->zyfylab=new CLabel(80,27,0,0,"按← →翻页",LABEL);
	this->sxfylab=new CLabel(80,28,0,0,"按↑ ↓选择数据",LABEL);
	
	this->cxbut =new CButton(95,10,10,2,"查  询",BUTTON);//14
	this->cancelbut=new CButton(32,27,10,2,"返  回",BUTTON);//15
...
int CUserQh::doAction()
{
	if(this->flag==14)
	{
		...
		for(it=quhaoList.begin();it!=quhaoList.end();it++)//将查询时间内的信息添加到容器里
		{
	...
	else if(this->flag==2)
	{
		...
		for(int i=0;i<7;i++)
		{
			arr[i]=new CButton(45,12+i*2,18,2,CTool::getTime(i),BUTTON);//绘制接下来7天的按钮
			arr[i]->show();
			count++;
		}
	...
	else if(this->flag==3)//从选中日期后面开始绘制
	{
		CButton *arr[10]={0};
